        ...
        <div className="flex w-full justify-between bg-gray-800 pt-4 pb-2">
            <div className="flex w-ful gap-2">
                ...
                <Tooltip text="모든 친구 목록을 보여줍니다" position="bottom">
                  <ButtonSet icon={faUsers} size="sm" label="모두" handleClick={() => {changeItemState("friendList")}} className="bg-gray-700 hover:bg-gray-900" />
                </Tooltip>
                ...
                {requestFriendData && requestFriendData?.list.length > 0 && (
                  <Tooltip text="받은 친구 요청 목록을 보여줍니다" position="bottom">
                    <ButtonSet icon={faUserPlus} size="sm" label={`받은 친구 ( ${requestFriendData?.list.length} )`} handleClick={() => {changeItemState("received-friend-request")}} className="bg-gray-700 hover:bg-gray-900" />
                  </Tooltip>
                )}
            </div>
        </div>
